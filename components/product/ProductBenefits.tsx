...
export function ProductBenefits({ product }: { product: Product }) {
  const { lang } = useI18n();
  const labels = product.content[lang].benefitLabels;
  ...
  return (
    <section className="relative border-y border-white/5 py-16">
      <div className="container-x">
        ...
        >
          {product.benefitIcons.map((Icon, i) => (
            <motion.div
              key={i}
            ...
            >
              ...
              <span className="font-heading text-sm font-semibold text-white">{labels[i]}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
